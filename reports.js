(function () {
  ...
  function getFilteredGroups() {
    let records = getRecordsForUser(session.email);
    if (dateFrom.value) {
  ...
  function openDetailModal(group) {
    detailModalTitle.textContent = `Detalle del ${formatDateShort(group.dateObj)}`;
  ...
  function render() {
    ...
    if (currentGroups.length === 0) {
      reportBody.innerHTML = '<tr class="empty-row"><td colspan="5">No hay registros en el rango seleccionado.</td></tr>';
      return;
    }
  ...
  exportBtn.addEventListener('click', () => {
    ...
    exportRowsToExcel(rows, `mis_reportes_${session.email}.xlsx`, 'Mis reportes');
  });
